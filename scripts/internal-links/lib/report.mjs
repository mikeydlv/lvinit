...
export const PROHIBITED_ACTIONS = [
  ...
  "modify the GSC Opportunity Agent's or the Fact-Decay Agent's reports or scoring",
...
export function buildMarkdownReport({ analysis, config, meta }) {
  ...
  L.push(
    `Mode: **${analysis.mode}**. ` +
      (analysis.mode === "dry-run"
        ? "Nothing was written, committed or pushed — this is what the agent *would* do."
        : "Safe links were applied, validated and shipped; everything else was reported.")
  );
  L.push("");
  ...
  L.push(`| Editorial links in the graph | ${t.editorialLinks} |`);
  L.push(`| Candidate links examined | ${t.candidatesExamined} |`);
  L.push(`| Opportunities detected | ${t.opportunitiesDetected} |`);
  L.push(`| **Auto-executed** | **${t.autoExecuted}** |`);
  L.push(`| Report-only (needs review) | ${t.needsReview} |`);
  ...
  if (analysis.autoExecuted.length === 0) {
    ...
    for (const c of analysis.autoExecuted) {
      ...
      L.push(`* **Confidence**: ${c.confidence} (priority ${c.priority})`);
      L.push(`* **Bridge sentence added**: no — the anchor is words that were already on the page`);
      L.push("");
      L.push("**The sentence, as it now reads:**");
      L.push("");
  ...
  L.push("## Needs review");
  L.push("");
  const review = analysis.needsReview.slice(0, config.output.maxReviewItems);
  if (review.length === 0) {
    L.push("Nothing. Every opportunity the agent found was either safe enough to do or not worth your time.");
    L.push("");
  } else {
    L.push(
      `${analysis.needsReview.length} opportunit${analysis.needsReview.length === 1 ? "y" : "ies"} need a person.` +
        (analysis.needsReview.length > review.length ? ` The ${review.length} most useful are below.` : "")
    );
    L.push("");
    ...
    for (const c of review) {
      ...
      L.push(`* **Proposed anchor**: “${c.anchor}” (confidence ${c.confidence}, priority ${c.priority})`);
      if (c.sentence) {
  ...
  L.push(
    "*Header, footer, nav and card-feed links are excluded on purpose — they point at every page and would make " +
      "nothing look like an orphan. These counts are contextual editorial links only. “Referring pages” is what " +
      "decides whether a page is weakly linked; “total links in” can be higher when one page links it twice.*"
  );
  L.push("");
  const listWeak = (rows, heading, empty) => {
    ...
    L.push("| Route | Published | Referring pages | Total links in | Linked from |");
    L.push("|---|---|---|---|---|");
    for (const p of rows.slice(0, config.output.maxWeakPagesListed)) {
      L.push(
        `| ${escapeCell(p.route)} | ${p.publishedAt ?? "—"} | ${p.uniqueReferrers.length} | ` +
          `${p.incomingEditorialCount} | ${escapeCell(p.uniqueReferrers.join(", ") || "nothing")} |`
      );
    }
    L.push("");
  };
    ...
    if (heavyDuplicates.length) {
      L.push(
        `**${heavyDuplicates.length} place(s) where one page links the same destination ` +
          `${config.output.duplicateReportThreshold}+ times.** Twice is normal in a long LVINIT guide and is not ` +
          `listed; this many starts to read as repetition. (${analysis.duplicateLinks.length} repeat link(s) in total ` +
          "across the site — all of them are in the JSON report.)"
      );
      L.push("");
