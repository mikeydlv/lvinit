...
> fix this file.
>
> **Last audited:** 2026-09-25 · **Branch:** `main` · **Live:** https://www.lvinit.com
>
...
app/
  ...
  guides/henderson-sport-social-grand-opening/page.tsx  /guides/henderson-sport-social-grand-opening — Local Feature, the confirmed Oct 16, 2026 grand opening of the $70M, 180,000 sq ft Henderson Sport & Social complex (formerly West Henderson Fieldhouse) at St. Rose and Maryland Pkwys (Story Page system)
  guides/new-build-vs-resale-las-vegas/page.tsx  /guides/new-build-vs-resale-las-vegas — Buyer Guide, companion to Mikey's video of the same name (Story Page system)
  guides/summerlin-vs-henderson-vs-southwest-las-vegas/page.tsx  /guides/summerlin-vs-henderson-vs-southwest-las-vegas — Comparisons, the third corner of the comparison cluster (Story Page system)
  guides/gholson-landing-affordable-housing-east-las-vegas/page.tsx  /guides/gholson-landing-affordable-housing-east-las-vegas — Local Feature, SNRHA's 121-unit, $32M affordable apartment community that opened Sept 22, 2026 at 2601 Sunrise Ave. in East Las Vegas (Story Page system)
  guides/las-vegas-mortgage-rates-19-month-high/page.tsx  /guides/las-vegas-mortgage-rates-19-month-high — Market Watch, the third installment in the mortgage-rate thread: Freddie Mac's PMMS jumped to 6.95% for the week of Sept 17, 2026 (its biggest one-week move of the run-up, highest since Jan 2025 per AP wire coverage), with daily trackers back above 7% by Sept 22 (Story Page system)
```

*(This file map had drifted behind the actual `app/guides/` tree before this entry — `new-build-vs-resale-las-vegas` and `summerlin-vs-henderson-vs-southwest-las-vegas` were already live routes, registered in `lib/content.ts` and `app/sitemap.ts`, just missing from this list. Added here for accuracy; not otherwise touched this run.)
...
| `/guides/las-vegas-mortgage-rates-approach-7-percent` | Market Watch guide | Live | Story Page system; built via the autonomous scheduled editorial-publishing routine as the direct sequel to `/guides/las-vegas-mortgage-rates-september-2026` (published 2026-09-07, week-of-Sept-3 print at 6.71%). Fresh news independently re-verified this run: Freddie Mac's own PMMS (`freddiemac.com/pmms` + `/archive`) shows three straight weekly increases since an Aug 20, 2026 low (6.65% → 6.66% → 6.71% → 6.76%), the 6.76% print for the week of Sept 10, 2026 corroborated as "highest since June 2025" (Bloomberg) / "over 14 months" (WTOP). Faster daily trackers (Zillow data cited by NerdWallet and Norada Real Estate) put the 30-year effectively at 7% by Sept 16, 2026 — deliberately not mixed with Mortgage News Daily's own, more volatile daily figure, to avoid implying three different methodologies disagree on one number; the piece instead explains the PMMS-lagging-average vs. daily-tracker distinction plainly. Also covers the Fed's Sept 16, 2026 quarter-point hike to 3.75%–4% (first in 3+ years, per CNBC) as backdrop/context only, explicitly not framed as the cause of a climb that predates the meeting by three weeks. Local price context reuses the already-sourced $475,000 August 2026 LVR median. No rate forecast anywhere in the piece. Financing-cost topic, not a place — no repo photography fits, and `C:\LVINIT\Images` was confirmed unreachable from this Linux cloud session (no `/mnt/c` mount) — so it originally carried a generated LVINIT editorial cover and a photoless `StoryHero`. **Updated 2026-09-17:** Mikey supplied an AI-generated image directly via GitHub upload (a hillside-overlook view of a generic Las Vegas residential street with the Strip skyline in the distance) — confirmed by him to be AI-generated, not a real photograph of any actual neighborhood, and not his own photography, so no Mikey photo credit runs. It now serves as both the `StoryHero` and the `/guides` card image (`hero/las-vegas-mortgage-rates-valley-homes.webp`, optimized from his 1672×941 PNG upload), with `imageMode: "editorial-cover"` kept in `lib/content.ts` so the card renders it honestly as a graphic rather than a claimed photograph, and a visible on-page disclosure banner runs immediately below the hero (same convention as the prior mortgage-rates piece's AI hero graphic and the Fiesta Henderson concept illustration). The superseded generated cover was deleted, along with a duplicate raw PNG upload. FAQPage + Article + Breadcrumb JSON-LD; cross-linked both directions with `las-vegas-mortgage-rates-september-2026` (new inline "Update:" paragraph + `relatedStories` entry on that page, pointing here), plus links to `las-vegas-home-prices-august-2026`, `las-vegas-starter-home-prices-2026`, `las-vegas-down-payment-assistance-programs-2026`, `what-500k-buys-in-las-vegas`, and `/search`; sitemap entry; registered in `lib/content.ts` `guides[]`. |
| `/guides/henderson-sport-social-grand-opening` | Local Feature guide | Live | Story Page system; built via the autonomous scheduled editorial-publishing routine, following through on the Henderson pillar guide's own Development Watch entry for this project (previously listed under its old name, "West Henderson Fieldhouse," with an "early October reported" caveat). Five independently direct-fetched sources this run (Nevada Business Magazine, News 3 Las Vegas, FOX5 Vegas, KTNV, KemperSports' own property page) confirm: Henderson Sport & Social, a 180,000 sq ft, two-level indoor sports and entertainment venue at 3375 St. Rose Pkwy (St. Rose and Maryland Pkwys, West Henderson), built as a ~$70M public-private partnership between the City of Henderson ($60M) and operator KemperSports ($10M); broke ground May 21, 2025 as the West Henderson Fieldhouse, renamed June 2026; soft opening Oct. 5, 2026, grand opening confirmed Friday, Oct. 16, 2026 (silent auction through Oct. 17 benefiting Cure 4 The Kids Foundation). Open to the public, no membership required; a fitness-membership/youth-league pricing figure (~$55/mo, ~$100/season, 20% resident discount) is explicitly sourced only to a pre-opening June 2026 report and flagged as not confirmed closer to opening. A dedicated section disambiguates this finished, opening building from the separate, still-conceptual Fiesta Henderson site redevelopment (`/guides/fiesta-henderson-redevelopment`), which already carried a similar disambiguation in the other direction — both pages now cross-link. UPDATED 2026-09-22: Mikey shot the building himself and supplied real, first-party photography (still pre-opening — construction equipment/materials visible), replacing the earlier generated placeholder. `StoryHero` now uses a real exterior elevation photo (`hero/henderson-sport-social-exterior-signage-hero.webp`), a real inline entrance-plaza/mini-golf figure sits in "What's actually inside" (`features/henderson-sport-social-entrance-plaza-mini-golf.webp`), and the `lib/content.ts` card image now points at the same real hero (generated cover file left in place but unreferenced). Article + Breadcrumb + FAQPage JSON-LD; cross-linked both directions with `/neighborhoods/henderson` (new inline link in its "West Henderson" section + `RelatedNeighborhood` card back, and the Development Watch entry for this project updated with the confirmed address/date/source) and `/guides/fiesta-henderson-redevelopment` (new inline link + `RelatedStories` entry on both pages), plus a link to `/guides/henderson-vs-southwest-las-vegas`; sitemap entry; registered in `lib/content.ts` `guides[]`. |
| `/guides/gholson-landing-affordable-housing-east-las-vegas` | Local Feature guide | Live | Story Page system; built via the autonomous scheduled editorial-publishing routine. Explicitly directed NOT to publish a third mortgage-rate piece this run (two open, unmerged PRs — #48 and #50 — already covered that "near/at a multi-month-high rate" thread; main only had the original `las-vegas-mortgage-rates-approach-7-percent` from that series), so a fresh, non-duplicative topic was researched instead. Covers the Sept 22, 2026 grand opening of Gholson Landing, a $32M, 121-unit affordable apartment community at 2601 Sunrise Ave. (East Las Vegas / "Downtown East"), built by the Southern Nevada Regional Housing Authority (SNRHA) and The Michaels Organization — one- to four-bedroom units, 600–1,300 sq ft, restricted to households at 30%–80% of area median income, already ~80% leased at opening. Named for Thomas Gholson, a former deputy executive director of the Las Vegas Housing Authority who once lived in public housing himself; carries his own real, attributed quote plus quotes from SNRHA's Frank Stafford and resident Dandy Tellez, all sourced verbatim to KTNV's on-the-ground coverage (Fox5 Vegas's two articles and a Review-Journal headline corroborate the event and the $32M figure; the RJ article itself returned a paywall/soft-block to direct fetch, so it's cited as corroborating by headline only, not as directly read — same discipline as the One Civic Center piece). Names the companion Beals-Henderson Pointe project (80 units, completed April 2026) and the $500M, ARPA-funded statewide Home Means Nevada Initiative as background, without asserting a project-specific funding breakdown that isn't in any source. States plainly, more than once, that this is income-restricted rental housing, not a path to homeownership — an honest framing choice on a real-estate site. Originally carried a generated LVINIT editorial cover and a photoless `StoryHero` (no repo photography existed and `C:\LVINIT\Images`, a Windows path, was confirmed unreachable from this Linux cloud session). Article + Breadcrumb JSON-LD (no FAQPage — matches the other recent Local Feature pieces, which don't carry one either); cross-linked both directions with `/guides/las-vegas-down-payment-assistance-programs-2026` (new `relatedStories` entry + a new inline paragraph there contrasting renting vs. down-payment-assistance paths), plus links to `/guides/las-vegas-home-prices-august-2026` and `/guides/las-vegas-starter-home-prices-2026`; sitemap entry; registered in `lib/content.ts` `guides[]`. |
| `/guides/las-vegas-mortgage-rates-19-month-high` | Market Watch guide | Live | Story Page system; built via the autonomous scheduled editorial-publishing routine as the third installment in the mortgage-rate thread, direct sequel to `/guides/las-vegas-mortgage-rates-approach-7-percent` (published 2026-09-17, week-of-Sept-10 print at 6.76%, daily trackers near 7% as of Sept 16). Fresh news independently re-verified this run via three separate fetches (Freddie Mac's own site, its official GlobeNewswire release, and its investor-relations mirror): the PMMS 30-year fixed average jumped to 6.95% for the week of Sept 17, 2026 — a 19-basis-point one-week move, the biggest of the four straight weekly increases in this run-up — independently corroborated as a "highest since Jan. 30, 2025" (~19-month) print by AP wire coverage (Alex Veiga, carried on the Arkansas Democrat-Gazette) and CNBC's own coverage of the same release; Bloomberg's headline on the same data independently corroborates the "fourth straight week" framing. A lower-quality aggregator's conflicting "highest since January 2026" framing was deliberately rejected as an uncorroborated, likely month/year mix-up. Daily-tracker "right now" context: four independent trackers (NerdWallet, Forbes Advisor, Mortgage Daily, Nadlan Capital Group) all put the 30-year back in the 7.02%–7.04% range on Sept 22, 2026; a same-day Zillow-sourced outlier at 7.16% was deliberately excluded. Reuses the already-sourced $475,000 August 2026 LVR median rather than re-deriving it (no LVR September report exists yet). Central payment-math example ($475,000 loan, 6.76% vs. 6.95% vs. the Sept 22 daily rate) is independently computed and clearly labeled hypothetical; AP's own national ($400,000 loan) payment-impact figure is cited only as attributed outside context, never substituted for the Las Vegas-specific math. No rate forecast anywhere in the piece. Financing-cost topic, not a place — no repo photography fits, and `C:\LVINIT\Images` was confirmed unreachable from this Linux cloud session (no `/mnt/c` mount). Unlike the two prior installments, this run carries no live, in-the-moment approval from Mikey to substitute AI-generated imagery (that was a one-time bypass each time, not a standing default), so it ships with a generated LVINIT editorial cover (`covers/las-vegas-mortgage-rates-19-month-high-editorial-cover.webp`, Market Watch motif) as the `/guides` card image and a photoless `StoryHero`. FAQPage + Article + Breadcrumb JSON-LD; cross-linked both directions with `las-vegas-mortgage-rates-approach-7-percent` (new inline "Update:" paragraph + `relatedStories` entry on that page, pointing here), plus links to `las-vegas-home-prices-august-2026`, `las-vegas-starter-home-prices-2026`, `las-vegas-down-payment-assistance-programs-2026`, `what-500k-buys-in-las-vegas`, and `/search`; sitemap entry; registered in `lib/content.ts` `guides[]`. **Note:** PR #48 (`las-vegas-mortgage-rates-hit-20-month-high`) covered this exact same Sept 17 PMMS print with near-identical framing and was closed as a superseded duplicate rather than merged — this piece is the one that shipped, at Mikey's direction. |
...
From Mikey's `C:\LVINIT\Images` source folder (pre-authorized as genuine
Mikey-owned LVINIT photography): `hero/las-vegas-starter-home-new-construction.webp`
(1724×970, 16:9, WebP q84) — a daylight street photo of two adjacent
new-construction homes in a Las Vegas subdivision (desert xeriscape, tile
roofs, rooftop solar on one home, builder yard signs), cropped from the
supplied original to the hero aspect ratio with a light saturation nudge only
(no HDR, no sky replacement — the source frame was already a clean, honest
daylight shot). Used as the real hero + OG image for
`/guides/las-vegas-starter-home-prices-2026`, replacing that piece's earlier
generated editorial cover. It's valley-wide new-construction imagery, not a
photo of a specific named neighborhood — matches the article's valley-wide
data and is captioned accordingly, not as evidence of any stat in the piece.

Supplied directly by Mikey on 2026-08-31 and confirmed as his own photography:
`hero/lennar-mockingbird-summerlin-new-construction-drone.webp` (source
1913×1072, cropped/upscaled to the standard 2400×1350 Story Page hero size —
no regrade) — an elevated street view of a Lennar new-construction street at
Mockingbird in Summerlin: finished stucco homes under scaffolding, open
timber framing mid-build, work trucks and dumpsters at the curb, valley and
mountains visible behind. Used as the real hero + OG image and `lib/content.ts`
card image for `/guides/las-vegas-new-home-sales-july-2026`, replacing that
piece's earlier generated editorial cover
(`covers/las-vegas-new-home-sales-editorial-cover.webp`, now orphaned — kept
in place, safe to prune). Mikey-owned, so the global footer credit covers it;
no per-image credit line.
...
- **Monument Hills** Local Feature guide — built via the autonomous scheduled
  editorial-publishing routine on a genuine, well-corroborated breaking story
  (Las Vegas Review-Journal, Sept 2, 2026, plus three independent local-TV
  corroborations). Covers the $94M/940-acre land sale for a planned ~6,000-home
  master-planned community in the far northwest valley, and leads with the
  jurisdiction clarification most coverage glosses over: this is City of Las
  Vegas land (a three-party BLM → City of Las Vegas → developer sale), not the
  separate incorporated city of North Las Vegas, even though the site borders
  areas the North Las Vegas guide already covers. Sets honest expectations —
  first homes aren't expected until spring 2028, so nothing is buyable now.
  Originally carried a generated LVINIT editorial cover and a photoless
  `StoryHero` (no real photography existed for this still-vacant site and
  `C:\LVINIT\Images` wasn't reachable that run). **Updated 2026-09-17, at
  Mikey's live request:** he uploaded his own drone and ground photography of
  the actual site directly to the repo (via GitHub web upload), plus a
  concept-plan exhibit page from the Monument Hills specific plan. The drone
  photo — existing Sunstone rooftops in the foreground, the vacant parcel
  stretching to the mountains beyond — now runs as both `StoryHero` and the
  `/guides`/homepage card image (`hero/monument-hills-sunstone-site-aerial-drone.webp`,
  `imageMode` unset/"photo," genuine Mikey photography); the superseded
  generated cover was deleted. A ground-level site photo
  (`features/monument-hills-site-desert-fenceline.webp`) runs inline near the
  deal section. The Neighborhood Park B concept exhibit
  (`features/monument-hills-neighborhood-park-concept-exhibit.webp`) runs
  inline in "What's actually planned," explicitly labeled "Site plan:
  Monument Hills specific plan" — planning material, not Mikey's photography
  and not a built park, captioned with the plan's own "conceptual, subject to
  change" note preserved. Also added: a `StoryVideo` section right after the
  lede embedding `youtube.com/watch?v=GQbCsZ_X3lc` via the existing
  click-to-play facade (poster until clicked, then autoplay from 0:00 — no
  page-load autoplay), poster from Mikey's own thumbnail design
  (`video-monument-hills-6000-homes-coming-soon.webp`). The video was
  privately scheduled to go live at 9am Pacific the day it was requested;
  YouTube's oEmbed endpoint returned 403 and no thumbnail resolved at the
  time of the edit (confirmed the fetch path itself was fine against a known
  public video), so the embed was wired up ahead of the video's own
  publish time rather than blocked on it — worth a quick click-through once
  live to confirm playback. No VideoObject JSON-LD added, matching the
  Henderson vs. Southwest precedent (upload date not independently
  verifiable from this session). Several other images Mikey uploaded in the
  same batch (house-style and additional park-concept exhibits, LVINIT
  chapter-card concepts, a labeled site-boundary map) weren't used here and
  were deliberately left in `public/images/` for future use, not deleted.
  Story Page system; Article + Breadcrumb JSON-LD; cross-linked both
  directions with `/neighborhoods/north-las-vegas` and
  `/guides/sandstone-tule-springs-north-las-vegas`, plus links to
  `/neighborhoods/southwest-las-vegas` and the July 2026 new-home-sales
  Market Watch piece; sitemap entry; registered in `lib/content.ts`
  `guides[]`.
...
- **Las Vegas Home Prices, August 2026** Market Watch guide — built via the
  ...
  `lib/content.ts` `guides[]`.
- **Sandstone at Tule Springs** Local Feature guide — built via the
  autonomous scheduled editorial-publishing routine on fresh, well-
  corroborated news: KB Home's own press release (Sept 4, 2026) plus
  independent Review-Journal reporting (Sept 8, 2026) on the opening of
  first-phase sales at a nearly 300-acre, 1,500-home master-planned
  community in North Las Vegas's Tule Springs area — homes from the high
  $300,000s, well under the valley's $475,000 resale median. A genuinely new
  topic (not a duplicate of the recent mortgage-rate or home-price Market
  Watch pieces, and a useful contrast to Monument Hills' not-buyable-until-
  2028 timeline). Originally carried a generated LVINIT editorial cover and a
  photoless hero (no real photography existed for this still-under-
  construction site and `C:\LVINIT\Images` wasn't reachable that run). On
  2026-09-14, Mikey uploaded five real images pulled directly from KB Home's
  own site; the community's model-home photo/rendering now serves as both
  hero and card (`hero/sandstone-tule-springs-model-homes-hero.webp`,
  `imageMode` "photo"), and the other four (pickleball, golf, hiking, Mount
  Charleston — KB Home's own "nearby lifestyle" marketing photos) are placed
  inline in a new "What's nearby, per KB Home" section via `StoryGallery`.
  None of these are Mikey's photography, so every placement carries a
  visible "Photo: KB Home" credit rather than relying on the footer's
  default photography credit; captions describe only what's visibly in each
  representative image rather than asserting it depicts a specific named
  course, trail, or Mount Charleston itself. The superseded generated cover
  was deleted. Story Page system; Article + Breadcrumb JSON-LD; cross-linked
  both directions with `/neighborhoods/north-las-vegas` and
...
- **Las Vegas Mortgage Rates, Approaching 7%** Market Watch guide — built via
  ...
  `lib/content.ts` `guides[]`.
- **Mortgage Rates Just Hit Their Highest Point Since January 2025** Market
  Watch guide — built via the autonomous scheduled editorial-publishing
  routine as the third installment in the mortgage-rate thread, direct
  sequel to `las-vegas-mortgage-rates-approach-7-percent`. Freddie Mac's own
  PMMS (verified via three independent fetches: its live site, its official
  GlobeNewswire release, and its investor-relations mirror) put the 30-year
  fixed average at 6.95% for the week of Sept 17, 2026 — up from 6.76%, a
  19-basis-point jump that's the biggest single-week move of the four
  straight weekly increases in this run-up — independently corroborated as
  a "highest since Jan. 30, 2025" (~19-month) print by AP wire coverage
  (Alex Veiga, via the Arkansas Democrat-Gazette) and CNBC, with Bloomberg's
  headline on the same release corroborating the "fourth straight week"
  framing. A conflicting "highest since January 2026" framing on one
  lower-quality aggregator was deliberately rejected as uncorroborated.
  Daily trackers (NerdWallet, Forbes Advisor, Mortgage Daily, Nadlan
  Capital Group) independently put the 30-year back at ~7.02%-7.04% on
  Sept 22, 2026. Reuses the already-sourced $475,000 August 2026 LVR median
  for the payment-math example. No real photography fits a financing-cost
  topic, `C:\LVINIT\Images` was confirmed unreachable from this Linux
  session, and no standing approval exists to substitute AI-generated
  imagery on an autonomous run (the prior two pieces' AI hero graphics were
  each a one-time, live, in-the-moment approval from Mikey), so it carries
  a generated LVINIT editorial cover and a photoless hero. Story Page
  system; FAQPage + Article + Breadcrumb JSON-LD; cross-linked both
  directions with `las-vegas-mortgage-rates-approach-7-percent` (new
  inline "Update:" paragraph + `relatedStories` entry on that page,
  pointing here), plus links to `las-vegas-home-prices-august-2026`,
  `las-vegas-starter-home-prices-2026`,
  `las-vegas-down-payment-assistance-programs-2026`, and
  `what-500k-buys-in-las-vegas`; sitemap entry; registered in
  `lib/content.ts` `guides[]`.
...
3. **North Las Vegas cluster:** `/neighborhoods/north-las-vegas` (pillar),
   `/guides/monument-hills-northwest-las-vegas` and
   `/guides/one-civic-center-north-las-vegas-redevelopment` (Local Feature
   children, cross-linked both directions), and
   `/guides/sandstone-tule-springs-north-las-vegas` (Local Feature child in
   the Tule Springs area, cross-linked both directions with the pillar and
   with Monument Hills).
...
12. **Monument Hills:** `/guides/monument-hills-northwest-las-vegas` — a
    standalone Local Feature, not a child of any existing pillar (the
    community itself doesn't exist yet, and its land sits inside the City of
    Las Vegas, a jurisdiction LVINIT has no pillar guide for). Cross-linked
    both directions with `/neighborhoods/north-las-vegas` (the closest
    existing pillar geographically, explicitly flagged as a different city
    government), plus links to `/neighborhoods/southwest-las-vegas` (the
    valley's other big growth corridor) and
...
2. **Henderson child guides** — Four Seasons Private Residences and the Water
   Street District (`/guides/water-street-district-henderson`, built
   2026-09-15) are both live. Lake Las Vegas and Green Valley Ranch are the
   strongest remaining candidates; each is already written up in the guide's
   roster and becomes a link by adding `href` to its entry in
   `lib/areas/henderson.tsx`. No placeholder cards ship in the meantime.
...
- **Henderson photography** — ✅ **Hero resolved 2026-08-27.** The guide runs
  ...
  serves as the OG image. Two caveats worth knowing. It is a **video still,
  cropped to 1903×716**, used at native resolution rather than upscaled, so it is
  lower-res than the Summerlin hero and will soften on a large high-DPI display;
  a full-res stills pass over the same flight would fix that. And the crop is
  load-bearing — see the PHOTOGRAPHY note in the route file. Any replacement
  Henderson hero with a bright upper-left needs its hero text contrast
  re-measured before it ships. **Water Street at ground level is resolved** —
  Mikey supplied two real photos of the district directly (2026-09-16), now
  live on `/guides/water-street-district-henderson`: a color gateway-arch
  shot as the hero/card image and a second color streetscape (MacKenzie's
  River Pub) as an inline figure. Still wanted, in rough priority order: Green Valley's
  mature tree canopy, the empty desert stretch on Lake Mead Pkwy toward Lake
  Las Vegas, and an Inspirada or Cadence streetscape with staked saplings as
  the counterpoint to Green Valley. (`neighborhood-henderson.jpg` remains one
  of the neutral placeholders in `public/images/README.md` and is not a photo
  of Henderson.)
